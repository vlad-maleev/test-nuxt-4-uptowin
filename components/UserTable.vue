<template>
  <div class="max-h-[520px] overflow-auto">
    <table class="w-full min-w-[760px] text-left text-sm">
      <thead class="sticky top-0 z-10 bg-slate-100 text-xs tracking-wide text-slate-500 uppercase">
        <tr>
      ...
      <tbody class="divide-y divide-slate-200">
        <tr v-for="user in users" :key="user.id" class="transition-colors hover:bg-slate-50">
          <td class="px-6 py-4">
            <div class="flex items-center gap-3">
              <div
                class="flex size-9 shrink-0 items-center justify-center rounded-full bg-emerald-100 font-semibold text-emerald-700"
              >
                {{ user.name.charAt(0) }}
              </div>
          ...
          <td class="px-4 py-4 whitespace-nowrap text-slate-500">{{ user.email }}</td>
          <td class="px-4 py-4 text-slate-700 tabular-nums">{{ user.age }}</td>
          <td class="px-4 py-4">
            <span
        ...
        <tr v-if="users.length === 0">
          <td colspan="5" class="px-6 py-16 text-center">
            <Users class="mx-auto mb-3 size-8 text-slate-400" aria-hidden="true" />
            <p class="font-medium text-slate-900">No users found</p>
            <p class="mt-1 text-sm text-slate-500">Try changing your search or role filter.</p>
          </td>
        </tr>
      </tbody>
    </table>
...
const roleClasses: Record<UserRole, string> = {
  ...
  manager: 'bg-amber-50 text-amber-700 ring-1 ring-amber-600/20',
  user: 'bg-slate-100 text-slate-600 ring-1 ring-slate-500/20'
}
